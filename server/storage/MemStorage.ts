...
export class MemStorage implements IStorage {
    ...
    [
      {
        ...
        content:
          "Sir, React hooks mein useEffect aur useLayoutEffect ka kya difference hai?",
        isFromAdmin: false,
      ...
      },
      {
        ...
        content:
          "Great question! useEffect runs after paint (asynchronous), while useLayoutEffect runs before paint (synchronous). Use useLayoutEffect when you need to measure DOM elements.",
        isFromAdmin: true,
      ...
      },
      {
        ...
        content:
          "Node.js module system samajh nahi aa raha - CommonJS vs ES Modules?",
        isFromAdmin: false,
      ...
      },
      {
        ...
        studentName: "Admin User",
        content:
          "CommonJS uses require() and module.exports (older style). ES Modules use import/export (modern standard). Node.js now supports both. For new projects, prefer ES Modules!",
        isFromAdmin: true,
  ...
  async getCoursesByCreator(creatorId: string) {
  ...
  async deleteModule(id: string) {
    ...
    Array.from(this.lectures.values())
      .filter((l) => l.moduleId === id)
      .forEach((l) => this.lectures.delete(l.id));
  }

  async getLecture(id: string) {
    return this.lectures.get(id);
  }
  async getModuleLectures(moduleId: string) {
  ...
  async getOrderStats() {
    ...
    return {
      totalRevenue: all.reduce((s, o) => s + o.amount, 0),
  ...
  async getCourseLectureCount(courseId: string) {
    const mods = Array.from(this.modules.values()).filter(
      (m) => m.courseId === courseId,
    );
    return mods.reduce(
      (count, m) =>
        count +
        Array.from(this.lectures.values()).filter((l) => l.moduleId === m.id)
          .length,
      0,
    );
  }
  ...
  async getLeaderboard() {
    ...
    return Promise.all(
      users.map(async (u) => {
        const completedRecs = Array.from(this.progressRecords.values()).filter(
          (p) => p.userId === u.id,
        );
  ...
  async getChatMessages(userId?: string): Promise<ChatMessage[]> {
    const all = Array.from(this.chatMessages.values()).sort(
  ...
  async markChatRead(userId: string) {
    Array.from(this.chatMessages.values())
      .filter((m) => m.studentId === userId && !m.isFromAdmin)
      .forEach((m) => {
  ...
  async getUnreadCount(userId: string): Promise<number> {
    return Array.from(this.chatMessages.values()).filter(
      (m) => m.studentId === userId && m.isFromAdmin && !m.isRead,
    ).length;
  }
}

...
export default function AdminLecturesPage({ courseId }: { courseId: string }) {
  ...
  const { data: courseData } = useQuery<{ course: Course }>({
  ...
  const handleFileSelect = async (file: File) => {
    ...
    setUploadDone(false);
    try {
      const { url, duration } = await uploadToCloudinary(
        file,
        setUploadProgress,
      );
      setLectureUrl(url);
  ...
  const saveLectureMutation = useMutation({
    mutationFn: async () => {
      const payload = {
        ...
        videoUrl: lectureUrl.trim() || undefined,
        duration: parseInt(lectureDuration) || 0,
      };
      if (lectureDialog.lecture) {
        const res = await apiRequest(
          "PATCH",
          `/api/lectures/${lectureDialog.lecture.id}`,
          payload,
        );
        if (!res.ok) throw new Error((await res.json()).message);
        return res.json();
        ...
        const res = await apiRequest(
          "POST",
          `/api/modules/${lectureDialog.moduleId}/lectures`,
          payload,
        );
        if (!res.ok) throw new Error((await res.json()).message);
        return res.json();
    ...
    onSuccess: () => {
      ...
      setUploadProgress(0);
      toast({
        title: lectureDialog.lecture ? "Lecture updated" : "Lecture added",
      });
    },
    onError: (err: any) =>
      toast({
  ...
  const openEditLecture = (moduleId: string, lecture: Lecture) => {
    ...
    setLectureUrl(lecture.videoUrl ?? "");
    setLectureDuration(String(lecture.duration));
    setUploadDone(false);
  ...
  return (
    <div className="max-w-4xl space-y-6">
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Add a module above to organize your lectures
            </p>
          </CardContent>
        </Card>
      ) : (
                  ...
                  <CollapsibleContent>
                    <div className="px-4 pb-4">
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                {lecture.videoUrl && (
                                  ...
                                  >
                                    {lecture.videoUrl.includes("cloudinary")
                                      ? "Cloudinary"
                            ...
                            <div className="flex gap-1 flex-shrink-0">
                              <Button
                                ...
                                className="h-7 w-7"
                                onClick={() =>
                                  openEditLecture(module.id, lecture)
                                }
                                data-testid={`button-edit-lecture-${lecture.id}`}
                              >
                      ...
                      <Button
                        ...
                        onClick={() => openAddLecture(module.id)}
      ...
      <Dialog
        ...
        onOpenChange={(open) => !open && closeLectureDialog()}
      >
        <DialogContent className="max-w-lg w-full overflow-hidden">
          <DialogHeader>
            <DialogTitle>
              {lectureDialog.lecture ? "Edit Lecture" : "Add Lecture"}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2 min-w-0">
            ...
            <div className="space-y-2">
              ...
              <Tabs defaultValue="upload">
                ...
                <TabsContent value="upload" className="mt-3 space-y-3">
                  {uploadDone ? (
                    <div className="flex items-start gap-2 p-3 rounded-lg bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-800 min-w-0 overflow-hidden">
                ...
                <TabsContent value="url" className="mt-3 space-y-2">
                  <Input
                    placeholder="https://youtube.com/embed/... or Cloudinary URL"
                    value={lectureUrl}
                    onChange={(e) => setLectureUrl(e.target.value)}
                    data-testid="input-lecture-url"
                  />
                  <p className="text-xs text-muted-foreground">
                    Paste a YouTube embed URL or any direct video URL
                  </p>
                </TabsContent>
              </Tabs>
            </div>
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-muted-foreground">
                Auto-filled when you upload a video
              </p>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={closeLectureDialog}>
              Cancel
            </Button>
            <Button
              onClick={() => saveLectureMutation.mutate()}
              disabled={
                !lectureTitle.trim() ||
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              Delete {deleteTarget?.type === "module" ? "Module" : "Lecture"}?
            </AlertDialogTitle>
            <AlertDialogDescription>
              "{deleteTarget?.name}" will be permanently deleted.
              {deleteTarget?.type === "module" &&
                " All lectures inside will also be deleted."}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
